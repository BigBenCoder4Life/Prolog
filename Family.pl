...
% female(X) is read X is not a male
female(X) :- \+male(X). 
...
step_father(X,Y) :- male(Y), \+parent(X,Y), spouse(Y,Z), parent(X,Z).
...
aunt(X,Y):- parent(X,A), sibling(A,B), spouse(B,Y).
...
relative(X,Y) :- ancestor(X,Z), ancestor(Y,A).
